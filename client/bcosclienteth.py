...
class BcosClientEth:
    ...
    def load_default_account(self):
        self.default_from_account_signer = Signer_ECDSA.from_privkey(self.privkey)

    def init(self):
        try:
            self.blockLimit = 500
            # check chainID
            common.check_int_range(client_config.groupid, BcosClientEth.max_group_id)
            # check group id
            common.check_int_range(client_config.fiscoChainId, BcosClientEth.max_chain_id)
            # check protocol
            if client_config.client_protocol.lower() not in BcosClientEth.protocol_list:
                raise BcosException("invalid configuration, must be: {}".
                                    format(''.join(BcosClientEth.protocol_list)))
            ...
            raise BcosException("init bcosclienteth failed, reason: {}".format(e))
    ...
    def getSystemConfigByKey(self, key):
        if key not in BcosClientEth.sysconfig_keys:
            raise ArgumentsError("invalid system key, must be {}"
                                 .format(BcosClientEth.sysconfig_keys))
